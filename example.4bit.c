...
int main(int argc, char *argv[])
{
	...
	for(size_t i = 0; i < DZ_MAT_SIZE; i++) {
		for(size_t j = 0; j < DZ_MAT_SIZE; j++) {
			score_matrix[i * DZ_MAT_SIZE + j] = (i & j) != 0 ? M : X;
		}
	}
	...
	/* pack query */
	char const *query = "\x1\x2\x1\x2\x8\x8\x2\x8\x1\x4\x1\x2\x8\x8\x8\x1\x2\x2\x1\x2\x8\x1";
	...
	ff[0] = dz_extend(
	    ...
	    "\x1\x2\x1\x2", 4, 0		/* reference-side sequence, its length, and node id */
	);

	/* branching paths: 1, 2 -> 3 */
	ff[1] = dz_extend(dz, q, &ff[0], 1, "\x8\x8\x4\x8", 4, 1);
	ff[2] = dz_extend(dz, q, &ff[0], 1, "\x1\x8\x2\x2", 4, 2);
	ff[3] = dz_extend(dz, q, &ff[1], 2, "\x1\x4\x1\x2", 4, 3);		/* "&ff[1], 2" indicates ff[1] and ff[2] are incoming nodes */

	/* insertion: -, 4 -> 5 */
	ff[4] = dz_extend(dz, q, &ff[3], 1, "\x8", 1, 4);
	ff[5] = dz_extend(dz, q, &ff[3], 2, "\x8\x8\x2\x8\x1", 5, 5);	/* "&ff[3], 2" indicates ff[3] and ff[4] are incoming nodes */

	/* SNVs: 6, 7, 8 -> 9 */
	ff[6] = dz_extend(dz, q, &ff[5], 1, "\x1", 1, 6);
	ff[7] = dz_extend(dz, q, &ff[5], 1, "\x2", 1, 7);
	ff[8] = dz_extend(dz, q, &ff[5], 1, "\x4", 1, 8);
	ff[9] = dz_extend(dz, q, &ff[6], 3, "\x2\x1\x2\x4\x4", 5, 9);
